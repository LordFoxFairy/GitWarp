...
Dossiers are stored in the root repository control plane, not inside each task worktree's source tree. Agents should read them through `gitwarp enter`, `gitwarp context`, `gitwarp board --verbose`, or the Web Console Sandboxes tab. This keeps task memory centralized for human review without polluting project files.
