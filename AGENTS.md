...
## Testing Guidelines
Behavior changes must include or update `tests/test_gitwarp.py`. Tests should create temporary Git repositories and exercise public CLI behavior such as `scan`, `summon`, `statusline`, and `collapse`. Always run both skill validation and unittest discovery before claiming the skill is ready.
