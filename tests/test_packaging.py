...
class PluginStructureTests(unittest.TestCase):
    ...
    def test_web_source_encodes_github_like_worktree_experience(self) -> None:
        ...
        self.assertIn("api.getState(selectedProject?.repo_root)", app)
        ...
        self.assertIn("contents with line numbers", code_panel)
        self.assertIn("splitFileLines", code_panel)
        self.assertIn("formatLineCount", code_panel)
        ...
        self.assertIn('runCommand("Create task"', app)
        ...
        self.assertIn("TaskCreateInput", api)
        self.assertIn("createTask", api)
        self.assertIn("/api/task/create", api)
        self.assertIn("addRepository", api)
        self.assertIn("/api/add", api)
        self.assertIn("removeWorktree", api)
        self.assertIn("/api/remove", api)
        self.assertIn("matching dossier", dossier_panel)
        self.assertIn("Finish Merged Task", dossier_panel)
        self.assertIn("Manual status", dossier_panel)
        self.assertIn("Remove Worktree", dossier_panel)
        ...
        self.assertIn("shell_command", action_panel)
        self.assertIn("cd command", action_panel)
        ...
        self.assertIn("classification_basis", types)
        self.assertIn("matrix?: MatrixPayload", types)
        self.assertIn("Review prune", branches_panel)
        self.assertIn("Delete local branch ref", branches_panel)
        self.assertIn('label: "Branches"', tabs)
        self.assertIn('label: "Sandboxes"', tabs)
        ...
        project_directory = (REPO_ROOT / "web" / "console" / "src" / "app" / "components" / "ProjectDirectory.tsx").read_text(encoding="utf-8")
        self.assertIn("Add current repository", project_directory)
        self.assertIn("Add repository path", project_directory)
        self.assertIn("matrixRows", picker)
        self.assertIn("Create base checkout", picker)
        self.assertIn("Base checkout only", picker)
        self.assertIn("taskWorktreesForBase", picker)
        self.assertIn("onSelectTaskCandidate", picker)
        self.assertIn("pendingTaskBranch", app)
        self.assertIn("Pending task candidate", action_panel)
    ...
    def test_install_scripts_have_version_and_path_guards(self) -> None:
        ...
        self.assertIn("codex plugin list --json", install_script)
        self.assertIn("codex plugin remove", install_script)
        ...
        self.assertIn("claude plugin update", claude_install_script)
        ...
        self.assertIn('install_cli="$REPO_ROOT/skills/gitwarp/scripts/install_cli.py"', install_script)
        self.assertIn('install_cli="$REPO_ROOT/skills/gitwarp/scripts/install_cli.py"', claude_install_script)
        self.assertNotIn("installed_path", install_script)
        self.assertNotIn('python3 "$installed_path/skills/gitwarp/scripts/install_cli.py"', install_script)
        self.assertNotIn('install_cli="$installed_path/skills/gitwarp/scripts/install_cli.py"', claude_install_script)
